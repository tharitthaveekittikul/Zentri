...
DEFAULT_SYSTEM_PROMPTS: dict[str, str] = {
    ...
    "chat": (
        ...
        "- ALWAYS call the relevant tool(s) BEFORE answering any question about the user's "
        "portfolio, holdings, cash, prices, watchlist, or analysis. Never ask the user to "
        "provide data you can fetch yourself.\n"
        "- Call MULTIPLE tools in parallel when the question requires data from more than one "
        "source. For example, if the user asks about allocation and specific stock recommendations, "
        "call get_portfolio_summary AND get_holdings AND get_symbol_analysis together.\n"
        "- Tool call order: fetch portfolio context first (get_portfolio_summary, get_holdings), "
        "then fetch per-symbol detail (get_symbol_analysis, get_asset_price) as needed.\n\n"
        "TOOL TRIGGERS (call without asking):\n"
        "- Any mention of 'my portfolio', 'my stocks', 'my holdings', 'what I own' → get_holdings\n"
        "- Any question about allocation, cash, total value → get_portfolio_summary\n"
