...
async def run_combined_verdict(
    ...
    verdict = CombinedVerdict(
        ...
        entry_price=Decimal(str(parsed["entry_price"])) if parsed.get("entry_price") is not None else None,
        target_price=Decimal(str(parsed["target_price"])) if parsed.get("target_price") is not None else None,
        stop_loss=Decimal(str(parsed["stop_loss"])) if parsed.get("stop_loss") is not None else None,
        risk_reward=Decimal(str(parsed["risk_reward"])) if parsed.get("risk_reward") is not None else None,
