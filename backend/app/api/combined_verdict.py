...
def _serialize(v) -> dict:
    return {
        ...
        "entry_price": float(v.entry_price) if v.entry_price is not None else None,
        "target_price": float(v.target_price) if v.target_price is not None else None,
        "stop_loss": float(v.stop_loss) if v.stop_loss is not None else None,
        "risk_reward": float(v.risk_reward) if v.risk_reward is not None else None,
